...
void Audio::playRandomCue(const char cue1[], const char cue2[], const char cue3[])
{
	if (soundBank == NULL)
		return;
	switch(rand()%3) {
		case 0:
			cueI = soundBank->GetCueIndex(cue1);
			break;
		case 1:
			cueI = soundBank->GetCueIndex(cue2);
			break;
		case 2:
			cueI = soundBank->GetCueIndex(cue3);
			break;
	}
	soundBank->Play( cueI, 0, 0, NULL );
}